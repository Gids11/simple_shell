#include "main.h"

/**
 * getargs - Gets a command from standard input.
 * @line: A buffer to store the command.
 * @exe_ret: The return value of the last executed command.
 *
 * Return: If an error occurs - NULL.
 *         Otherwise - a pointer to the stored command.
 */
char *getargs(char *line, int *exe_ret)
{
	size_t n = 0;
	ssize_t read;
	char *prompt = "$ ";

	if (line)
		free(line);

	read = _getline_(&line, &n, STDIN_FILENO);
	if (read == -1)
		return (NULL);
	if (read == 1)
	{
		hist++;
		if (isatty(STDIN_FILENO))
			write(STDOUT_FILENO, prompt, 2);
		return (getargs(line, exe_ret));
	}

	line[read - 1] = '\0';
	variable_change(&line, exe_ret);
	_handle_line(&line, read);

	return (line);
}

/**
 * callargs - Partitions operators from commands and calls them.
 * @args: An array of arguments.
 * @out: A double pointer to the beginning of args.
 * @exe_ret: The return value of the parent process' last executed command.
 *
 * Return: The return value of the last executed command.
 */
int callargs(char **args, char **out, int *exe_ret)
{
	int ret, index;

	if (!args[0])
		return (*exe_ret);
	for (index = 0; args[index]; index++)
	{
		if (strn_cmp(args[index], "||", 2) == 0)
		{
			free(args[index]);
			args[index] = NULL;
			args = _aliases_(args);
			ret = runargs(args, out, exe_ret);
			if (*exe_ret != 0)
			{
				args = &args[++index];
				index = 0;
			}
			else
			{
				for (index++; args[index]; index++)
					free(args[index]);
				return (ret);
			}
		}
		else if (strn_cmp(args[index], "&&", 2) == 0)
		{
			free(args[index]);
			args[index] = NULL;
			args = _aliases_(args);
			ret = runargs(args, out, exe_ret);
			if (*exe_ret == 0)
			{
				args = &args[++index];
				index = 0;
			}
			else
			{
				for (index++; args[index]; index++)
					free(args[index]);
				return (ret);
			}
		}
	}
	args = _aliases_(args);
	ret = runargs(args, out, exe_ret);
	return (ret);
}

/**
 * runargs - Calls the execution of a command.
 * @args: An array of arguments.
 * @out: A double pointer to the beginning of args.
 * @exe_ret: The return value of the parent process' last executed command.
 *
 * Return: The return value of the last executed command.
 */
int runargs(char **args, char **out, int *exe_ret)
{
	int ret, i;
	int (*builtin)(char **args, char **out);

	builtin = set_builtin(args[0]);

	if (builtin)
	{
		ret = builtin(args + 1, out);
		if (ret != EXIT)
			*exe_ret = ret;
	}
	else
	{
		*exe_ret = exec(args, out);
		ret = *exe_ret;
	}

	hist++;

	for (i = 0; args[i]; i++)
		free(args[i]);

	return (ret);
}

/**
 * args_handler - Gets, calls, and runs the execution of a command.
 * @exe_ret: The return value of the parent process' last executed command.
 *
 * Return: If an end-of-file is read - END_OF_FILE (-2).
 *         If the input cannot be tokenized - -1.
 *         O/w - The exit value of the last executed command.
 */
int args_handler(int *exe_ret)
{
	int ret = 0, index;
	char **args, *line = NULL, **out;

	line = getargs(line, exe_ret);
	if (!line)
		return (END_OF_FILE);

	args = _strtok_(line, " ");
	free(line);
	if (!args)
		return (ret);
	if (checkargs(args) != 0)
	{
		*exe_ret = 2;
		freeargs(args, args);
		return (*exe_ret);
	}
	out = args;

	for (index = 0; args[index]; index++)
	{
		if (strn_cmp(args[index], ";", 1) == 0)
		{
			free(args[index]);
			args[index] = NULL;
			ret = callargs(args, out, exe_ret);
			args = &args[++index];
			index = 0;
		}
	}
	if (args)
		ret = callargs(args, out, exe_ret);

	free(out);
	return (ret);
}

/**
 * checkargs - Checks if there are any leading ';', ';;', '&&', or '||'.
 * @args: 2D pointer to tokenized commands and arguments.
 *
 * Return: If a ';', '&&', or '||' is placed at an invalid position - 2.
 *	   Otherwise - 0.
 */
int checkargs(char **args)
{
	size_t i;
	char *cur, *nex;

	for (i = 0; args[i]; i++)
	{
		cur = args[i];
		if (cur[0] == ';' || cur[0] == '&' || cur[0] == '|')
		{
			if (i == 0 || cur[1] == ';')
				return (get_error(&args[i], 2));
			nex = args[i + 1];
			if (nex && (nex[0] == ';' || nex[0] == '&' || nex[0] == '|'))
				return (get_error(&args[i + 1], 2));
		}
	}
	return (0);
}
